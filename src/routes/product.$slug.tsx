...
function ProductPage() {
  ...
  const { image: catalogImage, bySlug: catalogBySlug } = useProducts();
  const catalogProduct = catalogBySlug(product.slug);
  ...
  return (
    <>
      ...
      <ProductInfo name={product.name} description={catalogProduct?.description ?? ""} />
...
/** Default copy used until an admin writes a description in /admin -> Products. */
...
/** Renders admin-authored copy: plain lines become paragraphs, "- " lines bullets. */
...
function ProductInfo({ name, description }: { name: string; description: string }) {
  return (
    <section className="container mx-auto px-4 py-10">
      ...
      <DescriptionBody text={description.trim() || DEFAULT_DESCRIPTION} />
    </section>
  );
}
